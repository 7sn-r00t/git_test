...
# Forked by 7sn-r00t for python workshop in CIC
import pygame
import random
import shelve
from os import path
...
# Function to save the last high score

def last_score():
        with open('LastHighScore.txt', 'w') as file:
            file.write(str(score))

# Function to load the last highscore
def load_highscore():
    try:
        with open('LastHighScore.txt', 'r') as file:
...
start_time = pygame.time.get_ticks()
GameOver = pygame.font.Font(None, 56)
#Function to display Game Over
def GameEndDisplay():
    GameO_Text = GameOver.render("Game Over! " , True, TEXT_COLOR)
    text_rect = GameO_Text.get_rect(center=(WIDTH // 2, HEIGHT // 2))
    WINDOW.blit(GameO_Text, text_rect)
...
while running:
    for event in pygame.event.get():
        ...
        if score == 10:
            ...
            BGCOLOR = (0,0,0)
            TEXT_COLOR = (220, 200, 100)
        if score == 15:
            FOOD_COLOR = (255, 0, 0)
            SNAKE_SPEED = 20
            SNAKE_COLOR = (0,72,100)
            BGCOLOR = (70,0,200)
        if score == 30:
            BGCOLOR = (0, 120, 0)
            TEXT_COLOR = (255, 255, 179)
